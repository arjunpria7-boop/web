  ...
  return (
    <div className="bg-slate-900/40 backdrop-blur-md border border-slate-700/50 rounded-lg overflow-hidden shadow-lg hover:shadow-purple-500/40 transition-shadow duration-300 transform hover:-translate-y-1">
      <div className="relative">
        <img src={entry.imageUrl} alt={entry.marketName} className="w-full h-28 sm:h-40 object-cover object-top" />
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity duration-300">
          <button
            onClick={() => onViewClick(entry.imageUrl, entry.marketName)}
            className="bg-purple-600 text-white font-bold py-1 px-3 sm:py-2 sm:px-4 text-sm rounded-full hover:bg-purple-700 transition-colors"
          >
            View
          </button>
        </div>
      ...
      <div className="p-2 sm:p-4 flex flex-col items-center text-center">
        <h3 className="text-base sm:text-xl font-bold text-purple-400">{entry.marketName}</h3>
